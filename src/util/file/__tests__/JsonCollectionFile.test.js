  ...
  describe('when not existing on disk', () => {
    ...
    describe('when constructed', () => {
      ...
      it('should return an instance of JsonCollectionFile', () => {
        expect(new JsonCollectionFile(mockFilePath)).toBeInstanceOf(JsonCollectionFile);
      });
    });
    ...
    describe('when read', () => {
      let file;
      beforeEach(() => {
        file = new JsonCollectionFile(mockFilePath);
      });
  ...
  describe('when existing on disk', () => {
    ...
    describe('find', () => {
      ...
      it('should return an item when one matches', async () => {
        await expect(file.find(item => item.id === '😀')).resolves.toMatchObject({ id: '😀' });
      });
    });
    ...
    describe('filter', () => {
      ...
      it('should return an item when one matches', async () => {
        await expect(file.filter(item => item.id)).resolves.toEqual(mockContent);
      });
    });
    ...
    describe('update', () => {
      ...
      it('should write the updated collection to disk', async () => {
        const callback = jest.fn(collection => collection.map(value => ({ ...value, touched: true })));
        await file.update(callback);
        expect(fs.files()[mockFilePath]).toEqual(
          JSON.stringify([{ id: '💩', touched: true }, { id: '😀', touched: true }], null, 2),
        );
      });
    });
    ...
    describe('remove', () => {
      ...
      it('should not throw an error', async () => {
        await expect(file.remove(item => item.id === removeItemId)).resolves.toBeUndefined();
      });
      it('should append an item to the collection', async () => {
  ...
  describe('validate', () => {
    ...
    describe('with no uniqueKeys specified', () => {
      it('should not throw an error when given a uniquely indexed collection', () => {
        expect(() => JsonCollectionFile.validate(uniquelyIndexedCollection)).not.toThrowError();
      });
      it('should not throw an error when given a non-uniquely indexed collection', () => {
        expect(() => JsonCollectionFile.validate(nonUniquelyIndexedCollection)).not.toThrowError();
      });
    });
    ...
    describe('with uniqueKeys specified', () => {
      ...
      it('should not throw an error when given a uniquely indexed collection', () => {
        expect(() => UniqueCollection.validate(uniquelyIndexedCollection)).not.toThrowError();
      });
      it('should throw an error when given a non-uniquely indexed collection', () => {
        expect(() => UniqueCollection.validate(nonUniquelyIndexedCollection)).toThrowError();
      });
    });
  });
});
